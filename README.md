...
- Snapshots and an automated backup job configured.

## Documentation
Detailed write-ups for each component live in the [`docs/`](docs/) folder. Start here if you want the deep dive on a specific piece:
- [Proxmox Setup](docs/proxmox-setup.md) — install, repository configuration, and secondary storage
- [pfSense Firewall](docs/pfsense.md) — the router/firewall VM, interface assignment, and network segmentation
- [WireGuard VPN](docs/wireguard-vpn.md) — encrypted remote access setup, port forwarding, and the key-mismatch debugging story
- [Ubuntu Server VM](docs/ubuntu-server.md) — the services host and static-IP configuration
- [Windows Server VM](docs/windows-server.md) — the Active Directory domain controller build

*(Each file is added as that part of the lab is documented — this list grows as the project does.)*

## Problems & Solutions
The most valuable part of this project. Each entry is a real issue, its root cause, and the fix — the same problem → diagnosis → fix → lesson structure used in professional incident reports.
...
## Tech Stack
Proxmox VE 9, Linux (Ubuntu Server, Debian), pfSense, WireGuard, LXC containers, Active Directory (in progress), bridged/segmented networking, DHCP/DNS, DuckDNS. Languages: Bash, with Python automation to follow in later phases.
