...
| Windows Server 2025 | VM | vmbr1 | Active Directory domain controller (isolated) |
...
## Documentation
Detailed write-ups for each component live in the [`docs/`](docs/) folder, numbered in build order. Start here if you want the deep dive on a specific piece:
- [01 — Proxmox Setup](docs/01-proxmox-setup.md) — install, repository configuration, storage, bridges, and backups
- [02 — pfSense Firewall](docs/02-pfsense.md) — the router/firewall VM, interface assignment, and network segmentation
- [03 — Ubuntu Server VM](docs/03-ubuntu-server.md) — the services host
- [04 — Windows Server VM](docs/04-windows-server.md) — the Active Directory domain controller build
- [05 — WireGuard VPN](docs/05-wireguard-vpn.md) — encrypted remote access, port forwarding, and the key-mismatch debugging story
- [06 — Pi-hole](docs/06-pihole.md) — DNS sinkhole, rollout strategy, and the encrypted-DNS bypass investigation
...
**Fix:** Disabled both enterprise repos in the new `.sources` format (the deb822 format uses an `Enabled: false` line, not a commented-out `deb` line) and confirmed the free `pve-no-subscription` repo was active. `apt update` then ran clean.
**Lesson:** Repository formats change between major OS versions; verify the current format rather than copying an older guide's commands.
*Full write-up: [01 — Proxmox Setup](docs/01-proxmox-setup.md)*
...
**Lesson:** Reused drives carry old encryption/partition state; always confirm the target device before wiping, and the web UI's Disks → Directory tool is cleaner than manual `fdisk`.
*Full write-up: [01 — Proxmox Setup](docs/01-proxmox-setup.md)*
...
**Lesson:** Proxmox networking uses a staged "pending" config; changes aren't live until applied. When the web UI gives a vague error, `qm start <id>` surfaces the real cause.
*Full write-up: [01 — Proxmox Setup](docs/01-proxmox-setup.md)*
...
**Lesson:** The Windows boot-from-CD prompt is unreliable to catch; controlling boot order is the deterministic fix.
...
**Lesson:** In WireGuard, each side's `[Interface]` holds its own private key and each `[Peer]` holds the other side's public key. Identical keys in both fields is the classic silent-failure cause; `wg show` confirms the live state.
...
**Lesson:** If a resolver shows queries but zero blocks, check *which* queries are arriving — heartbeats reaching it while browsing traffic doesn't is the signature of an encrypted-DNS bypass. This same DoH-evading-DNS-filtering problem is a live issue for SOC and network teams in production.
*Full write-up: [06 — Pi-hole](docs/06-pihole.md)*
...
## Tech Stack
Proxmox VE 9, Linux (Ubuntu Server, Debian), pfSense, WireGuard, Pi-hole, LXC containers, Active Directory (in progress), bridged/segmented networking, DHCP/DNS, DNS sinkholing/filtering, DuckDNS. Languages: Bash, with Python automation to follow in later phases.
